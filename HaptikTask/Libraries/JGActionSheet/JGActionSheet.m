...
+ (instancetype)sectionWithTitle:(NSString *)title message:(NSString *)message buttonTitles:(NSArray *)buttonTitles buttonImages:(NSArray *) buttonImages buttonStyle:(JGActionSheetButtonStyle)buttonStyle {
    return [[self alloc] initWithTitle:title message:message buttonTitles: buttonTitles buttonImages:buttonImages buttonStyle:buttonStyle];
}
...
- (instancetype)initWithTitle:(NSString *)title message:(NSString *)message buttonTitles:(NSArray *)buttonTitles buttonImages: (NSArray *) buttonImages buttonStyle:(JGActionSheetButtonStyle)buttonStyle {
    self = [super init];
    ...
    if (self) {
        ...
        if (buttonTitles.count) {
            ...
            for (NSString *str in buttonTitles) {
                NSString *imageStr = [buttonImages objectAtIndex: index];
                
                JGButton *b = [self makeButtonWithTitle:str image:imageStr style:buttonStyle];
...
- (void)setButtonStyle:(JGActionSheetButtonStyle)buttonStyle forButton:(UIButton *)button {
    ...
    if (buttonStyle == JGActionSheetButtonStyleDefault) {
        font = [HTThemeAPI actionSheetLabelFont];
        titleColor = [UIColor blackColor];
        
        backgroundColor = [UIColor whiteColor];// [UIColor colorWithWhite:0.95f alpha:1.0f];
        borderColor = [UIColor colorWithWhite:0.9f alpha:1.0f];
    }
    else if (buttonStyle == JGActionSheetButtonStyleCancel) {
        font = [HTThemeAPI actionSheetCancelFont];
...
- (JGButton *)makeButtonWithTitle:(NSString *)title image:(NSString *)imageUrl style:(JGActionSheetButtonStyle)style {
    JGButton *b = [[JGButton alloc] init];
    
    if (imageUrl) {
        UIImage *image = [UIImage imageNamed: imageUrl];
        b.imageView = [[UIImageView alloc] initWithImage: image];
        b.imageView.userInteractionEnabled = NO;
        [b addSubview: b.imageView];
    }
...
#pragma clang diagnostic pop
    ...
    for (JGButton *button in self.buttons) {
        ...
        if (nil != button.imageView)
        {
            CGSize tsz = [self sizeOfLabel: button.titleLabel];
            CGSize isz = button.imageView.image.size;
            CGFloat w = width;
            CGFloat ihoffset = (buttonHeight - isz.height)/2.0;
            CGFloat leftGap = (w - (tsz.width + isz.width + titleImageOffset))/2.0;
            button.imageView.frame = CGRectMake(leftGap, ihoffset, isz.width, isz.height);
            CGFloat thoffset = 1.0;
            button.imageEdgeInsets = UIEdgeInsetsMake(0, 0, 0, titleImageOffset);
            button.titleEdgeInsets = UIEdgeInsetsMake(thoffset, titleImageOffset, 0, 0);
        }
        height += buttonHeight;
        ++index;
        ...
        if (index < [self.buttons count])
        {
            UIView *v = [[UIView alloc] initWithFrame: CGRectMake(0, height - 0.5, width, 0.5)];
            [v setBackgroundColor: [HTThemeAPI separatorColor]];
            [self addSubview: v];
        }
    }
...
{
    CGRect rect = [label.text boundingRectWithSize: label.frame.size
...
- (instancetype)initWithSections:(NSArray *)sections {
    ...
    if (self) {
        ...
        self.backgroundColor = [UIColor colorWithWhite:0.0f alpha: kViewOpacity];
